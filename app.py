...
if submit_button:
    if not raw_input.strip():
        ...
        for company in company_list:
            if company in st.session_state.result_cache:
                final_results.append(st.session_state.result_cache[company])
            else:
        ...
        if companies_to_fetch:
            # ==================================
            # Tavily (マルチスレッドで並列高速化)
            # ==================================
            status.text("会社概要および九州拠点を検索中... (並列処理により高速化中)")
            fetched_data = []

            def fetch_wrapper(comp):
                data = search_company(comp, tavily_api_key)
            ...
            # ==================================
            # Gemini (マルチスレッドで並列高速化)
            # ==================================
            status.text("AIによる会社概要・社名照合中... (並列処理により高速化中)")
            company_map = {}
            chunk_size = 5
