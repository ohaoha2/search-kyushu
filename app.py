...
def analyze_company(company_name, gemini_key):
    ...
    prompt = f"""
    あなたは企業の所在調査および営業戦略のプロフェッショナルです。
    ...
    5. "reasoning" は長々と書かず、1〜2文で簡潔にまとめてください。
    6. この企業への営業アプローチで使えそうなキーワードや業界特性（3〜5個）を "sales_keywords" の配列として抽出してください。
    ...
    {{
        ...
        "sales_keywords": ["キーワード1", "キーワード2", "キーワード3"]
    }}
    """
        ...
        with st.spinner(f"「{company_name}」をAIが分析中...少々お待ちください。"):
            try:
                ...
                if result.get('is_found'):
                    ...
                    # 拠点がない場合でも営業キーワードがあれば役立つので表示
                    keywords = result.get('sales_keywords', [])
                    if keywords:
                        st.markdown("### 🔑 参考営業キーワード")
                        keywords_md = " ".join([f"`{kw}`" for kw in keywords])
                        st.markdown(keywords_md)
