...
# ==========================================
# 1. 検索エンジンの実行関数（URLリストも一緒に返すように改良）
# ==========================================
            ...
            if comp in st.session_state.result_cache:
                ...
                try:
                    ...
                    if err:
                        ...
                        # 【フォールバック機構】もしAIが公式サイトを見つけられなかった場合、検索結果の先頭URLを自動補完する
                        if not res.get('official_url') or res.get('official_url') in ["null", ""]:
                            for r in raw_results:
                                url_lower = r['url'].lower()
                                # Wikipediaや求人サイトなどを除外して、それらしいものを拾う
                                if not any(x in url_lower for x in ["wikipedia", "job", "wantedly", "en-japan", "rikunabi", "mynavi"]):
                                    res['official_url'] = r['url']
                                    break
                            # それでも見つからなければ最初のURLを強制セット
...
if "batch_results" in st.session_state and st.session_state["batch_results"]:
    ...
    # 一括コピー用テキストエリア（Gemini風のコピーがしやすいようにTSV形式で表示）
