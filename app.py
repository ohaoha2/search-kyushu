...
==================================================
【official_url】（会社概要URLの厳格な選定）
==================================================
入力会社名の対象法人自身の純粋な「会社概要・企業情報ページ」のURLを記載してください。
【厳格ルール】
- URLのパスに /company, /about, /corporate, /profile 等が含まれる固定の会社概要ページを最優先で選んでください。
- ニュース、プレスリリース、社名変更告知ページはofficial_urlとして絶対に使わないでください！
...
・【入力会社名】が【現在の最新の正式法人名】と一致している場合：
  → **絶対に「〇」** とだけ出力してください。（過去に社名変更履歴があっても、現在の名前で入力されていれば100%「〇」です）

・【入力会社名】が『過去の旧社名』であり、現在は新しい社名に変更されている場合のみ：
  → テキスト全体を1つのMarkdownリンクにし、以下のフォーマットで出力してください。
  フォーマット： [✕ [変更年月日] 『現在の新社名』へ変更](社名変更の根拠URL)
  （例： [✕ 2023年4月1日に 『ニデック株式会社』へ変更](https://www.nidec.com/jp/news/2023/...) ）
  （例： [✕ 2024年4月1日に 『クララ株式会社』へ変更](https://www.clara.co.jp/press/...) ）
  ※【リンクURLの厳守】: 社名変更の根拠URLには、Q1検索結果にある「社名変更のお知らせ」「プレスリリース」「沿革」「ニュース」のURLを優先して割り当ててください。（なければ会社概要のURL）
  ※【日付ルール】: テキスト内に明記されている正確な年月日（例: 2023年4月1日）がある場合のみ日付を記載し、不確実なら日付なしで「[✕ 『現在の新社名』へ変更](URL)」としてください。
    ...
    if not raw_input.strip() or raw_input.strip() == "株式会社○○○○":
        ...
        if companies_to_fetch:
            ...
            for company in companies_to_fetch:
                ...
                # ★【Python側自動補正】入力社名が「現在の新社名」なら、AIが社名変更文脈を出していても物理的に100%「〇」へ補正する
                ...
                if not (company_match == "〇" or company_match.startswith("〇")):
                    if "へ変更" in company_match or "に変更" in company_match:
                        new_name_matches = re.findall(r"[『「]([^『「』」]+?)[』」]\s*?[へに]変更", company_match)
                        if new_name_matches:
                        ...
                        extracted_core = info_extracted["core"] if info_extracted["core"] else extracted_new_name

                        if (norm_input == extracted_new_name or 
                            (input_core and input_core == extracted_core) or 
                            candidate_entity_relation(company, extracted_new_name) == "match" or
                            (input_core and input_core in extracted_new_name)):
                            company_match = "〇"
